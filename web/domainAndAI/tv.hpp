...
public:
    ...
    Id = _productId; name = _name; screenSize = _screenSize; screenTech = _screenTech, resolution = _resolution, isThreeD = _isThreeD; isHDR = isHDR;};
    ...
    std::vector<std::string> virtual getProductFeaturesForCompare() {std::vector<std::string> vect{std::to_string(screenSize), screenTech,
        resolution, std::to_string(int(isThreeD)), std::to_string(int(isHDR))}; return vect;};
private:
    int screenSize;
