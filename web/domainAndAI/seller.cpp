...
void Seller::addOffer(int id, int productId, float unitPrice, int sellerId, int amount, std::string productName) 
{
...
{
    for(int i = 0; i < offers.size(); i++)
        if(offers[i].getProductId() == productId && offers[i].getSellerId() == Id)
            return false;
    return true;
}
