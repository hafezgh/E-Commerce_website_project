...
void SiteInterface::executePostCommand()
{
    ...
    if(parsedCommand[1] == "offer")
    {
        ...
        sellers[getSellerIndex(loggedInUserId)].addOffer(++offerCounter, atoi(parsedCommand[4].c_str()),
                 atoi(parsedCommand[6].c_str()), loggedInUserId, atoi(parsedCommand[8].c_str()), productName);
        return;
    }
    ...
    if(parsedCommand[1] == "chargeWallet")
    {
        ...
        if(!buyerLoggedIn)
            throw PermissionDeniedException();
        if(atof(parsedCommand[3].c_str()) <= 0)
            throw BadRequestException();
        buyers[getBuyerIndex(loggedInUserId)].chargeWallet(atof(parsedCommand[3].c_str()));
        return;
    }
...
void SiteInterface::executeGetCommand()
{
    if(parsedCommand[1] == "offers")
    {
        ...
        if(parsedCommand[2] != "?" && parsedCommand[3] != "order" && parsedCommand[5] != "field")
            throw NotFoundException();
        if(!(parsedCommand[3] == "ASCEND" || parsedCommand[3] == "DESCEND"))
            throw NotFoundException();
    ...
    if(parsedCommand[1] == "getMyOffers")
    {
        if(parsedCommand.size() != 2)
            throw NotFoundException();
        if(!sellerLoggedIn)
    ...
    if(parsedCommand[1] == "getOffersOnProduct")
    {
